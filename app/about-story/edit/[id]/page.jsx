...
export default function EditAboutStoryItem({ params }) {
    ...
    useEffect(() => {
        const fetchAboutStoryItem = async () => {
            try {
                const response = await fetch(`/api/about-story/${id}`);
                if (!response.ok) {
                    throw new Error('Failed to fetch about story item');
                }
                ...
                console.error('Error fetching about story item:', error);
                toast.error('Failed to load about story item');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const onSubmit = async (formData) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to update about story item');
            }
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-100 p-4">
            <h1 className="text-2xl font-bold mb-4 text-center">Edit About Story Item</h1>
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 space-y-4">
                <div>
                    ...
                    <textarea
                        ...
                        placeholder="Enter about story item description"
                    />
