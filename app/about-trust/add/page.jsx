...
export default function AddAboutTrustItem() {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const response = await fetch('/api/about-trust', {  // Updated endpoint
        method: 'POST',
        body: aboutData,
      });
      ...
      router.push('/about-trust'); // Updated redirection path
    } catch (error) {
      toast.error(`Failed to add about item: ${error.message}`);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="bg-white shadow-lg rounded-lg max-w-md w-full p-6">
        <h1 className="text-2xl font-bold mb-4 text-center text-black">Add New About Item</h1>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder="Enter about item description"
            />
          ...
          <div>
            ...
            >
              {isSubmitting ? 'Adding...' : 'Add About Item'}
            </button>
          </div>
        </form>
      </div>
