...
export default function EditAboutTrustItem({ params }) {
    ...
    useEffect(() => {
        const fetchAboutTrustItem = async () => {
            try {
                const response = await fetch(`/api/about-trust/${id}`);
                if (!response.ok) {
                    throw new Error('Failed to fetch about trust item');
                }
                ...
                console.error('Error fetching about trust item:', error);
                toast.error('Failed to load about trust item');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const onSubmit = async (formData) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to update about trust item');
            }

            toast.success('About item updated successfully!');
            router.push('/about-trust');
        } catch (error) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-100 p-4">
            <h1 className="text-2xl font-bold mb-4 text-center">Edit About Trust Item</h1>
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 space-y-4">
                <div>
                    ...
                    <textarea
                        ...
                        placeholder="Enter about trust item description"
                    />
