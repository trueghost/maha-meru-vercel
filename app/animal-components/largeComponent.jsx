...
import smallplant from '../../public/images/animalpage/animal1.svg'; // Default image for fallback
import { useLanguage } from "../context/languageContext";

// eslint-disable-next-line react/display-name
const Largecomponent = forwardRef(({ buttonTitles, animalItems, setActiveButton, setNextTitle, nextTitle, nextTitleActive, activeTitle, setNextTitleActive }, ref) => {
  const [activeButtonTitle, setActiveButtonTitle] = useState("Animal Bedding Care");
      ...
      <div className="hidden lg:flex w-full flex-col items-center space-y-5">
        <div className="flex w-full max-w-4xl justify-between space-x-4 mb-4">
          {buttonTitles.slice(0, 3).map((title, index) => (
            <button
              key={index}
              className="text-[#FBF6F1] rounded-lg text-[22px] font-poppinsSB py-3 flex-1 z-20 max-w-[300px]"
        ...
        <div className="flex w-full max-w-2xl justify-center space-x-4">
          {buttonTitles.slice(3).map((title, index) => (
            <button
              key={index + 3}
              className="text-[#FBF6F1] rounded-lg text-[22px] font-poppinsSB py-3 flex-1 max-w-[300px]"
              style={{ backgroundColor: activeButtonTitle === title.original ? "#897662" : "#314559" }}
              onClick={() => handleClick(title.original)}
              data-aos="fade-up"
            >
      ...
      <div className="lg:hidden w-full flex justify-start mt-[-80px] font-poppinsSB overflow-x-auto overflow-y-hidden scrollbar-hide">
        <div className="flex space-x-4 px-2 pl-20">
          {buttonTitles.map((title, index) => {
            ...
            const imageIndex = index % animalItems.length; // This ensures repetition
            // console.log(animalItems)
            const imageSrc = animalItems.length > 0 ? animalItems[imageIndex].animalSmallImage : smallplant;
