...
export default function AddAnimalTitleItem() {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      toast.success('Animal title added successfully!');
      reset(); // Reset the form fields
      router.push('/animal-titles'); // Redirect to the appropriate page
    } catch (error) {
      toast.error(`Failed to add animal title: ${error.message}`);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="bg-white shadow-lg rounded-lg max-w-md w-full p-6">
        <h1 className="text-2xl font-bold mb-4 text-center text-black">Add New Animal Title</h1>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            >
              {isSubmitting ? 'Adding...' : 'Add Animal Title'}
            </button>
          </div>
        </form>
      </div>
