...
const Page = () => {
  ...
  const [activeTitle, setActiveTitle] = useState("Animal Bedding Care");
      ...
      useEffect(() => {
        const fetchTitlesAndSubtext = async () => {
          try {
            ...
            await Promise.all([
              setTitleAndSubtext("Animal First", setTitle1),
              setTitleAndSubtext("Animal Second", setTitle2, setSubText),
            ]);
          } catch (error) {
        ...
        const fetchAnimalItemsAndTitles = async () => {
          try {
            // Fetch agriculture titles
            const titleResponse = await fetch("/api/animal-titles");
            if (!titleResponse.ok) throw new Error("Failed to fetch animal titles");
            const titleData = await titleResponse.json();
            setAnimalTitlesItems(titleData.data);
            ...
            const itemsResponse = await fetch("/api/animal-admin");
            if (!itemsResponse.ok) throw new Error("Failed to fetch animal items");
            const itemsData = await itemsResponse.json();
            setAnimalItems(itemsData.data);
...
<div ref={secondSectionRef} className="relative pt-20 xl:pt-0">
  ...
  ></div>
      ...
      <Animal title2={title2} subtext={subtext} />
      <Largecomponent buttonTitles={buttonTitles} animalItems={animalItems} setActiveButton={setActiveTitle} setNextTitle={setNextTitle} nextTitle={nextTitle} nextTitleActive={nextTitleActive} ref={largeComponentRef} activeTitle={activeTitle} setNextTitleActive={setNextTitleActive}/>
      <AnimalDynamic sections={sections} animalTitlesItems={animalTitlesItems} translatedSeeProductsText={translatedSeeProductsText} translatedNextTitle={translatedNextTitle} products={products}
      ...
      <Agriculturefour activeTitle={activeTitle}/>
      <Agriculturefinal activeTitle={activeTitle} nextTitle={nextTitle} setNextTitleActive={setNextTitleActive} scrollToLargeComponent={scrollToLargeComponent}/> */}
