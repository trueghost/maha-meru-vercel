...
import { getAllFormItems, addFormItem } from '../../../lib/formHandle';
...
export const POST = async (req) => {
  try {
    ...
    const { name, email, number, interests, description } = formData;
    ...
    if (!name || !email || !number) {
      console.warn('Validation failed: Name, Email, and Phone Number are required'); // Log a warning if validation fails
      return NextResponse.json({
        success: false,
    ...
    const formItem = {
      name,
      email,
      number,  // Store full phone number
      interests: interests.join(','), // Convert array of interests into a comma-separated string
      description,
    };
