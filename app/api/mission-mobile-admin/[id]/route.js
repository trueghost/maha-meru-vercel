...
import {
    getMissionMobileItemById,
    updateMissionMobileItem,
    deleteMissionMobileItem
} from "../../../../lib/missionMobile";
...
export const GET = async (req, { params }) => {
    const { id } = params;
    try {
        const item = await getMissionMobileItemById(id);
...
export const PUT = async (req, { params }) => {
    ...
    try {
        // Fetch the existing mission mobile item
        const existingItem = await getMissionMobileItemById(id);
        ...
        let updatedLogoPath = existingItem.logo;
        let updatedSmallLogoPath = existingItem.smallLogo;
        ...
        const logoFile = { field: 'logo', path: updatedLogoPath };
        const smallLogoFile = { field: 'smallLogo', path: updatedSmallLogoPath };
        ...
        const handleImageUpload = async (fieldName, existingPath) => {
            ...
            if (file && file instanceof File) {
                ...
                return `/uploads/missionMobile/${fileName}`;
            }
            return existingPath;
        };
        ...
        // Process the logo and small logo
        updatedLogoPath = await handleImageUpload(logoFile.field, updatedLogoPath);
        updatedSmallLogoPath = await handleImageUpload(smallLogoFile.field, updatedSmallLogoPath);

        // Update the mission mobile item
        await updateMissionMobileItem(id, {
            ...body,
            ...
            smallLogo: updatedSmallLogoPath,
        ...
        return NextResponse.json({ success: true, message: 'Mission mobile item updated successfully!' });
    } catch (error) {
...
export const DELETE = async (req, { params }) => {
    ...
    try {
        // Fetch the mission mobile item to get the image paths
        const item = await getMissionMobileItemById(id);

        // Delete the item from the database
        await deleteMissionMobileItem(id);
        ...
        return NextResponse.json({ success: true, message: 'Mission mobile item deleted successfully!' });
    } catch (error) {
        return NextResponse.json({ success: false, message: error.message });
    }
};
