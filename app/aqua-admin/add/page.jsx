...
export default function AddAquaItem() {
  ...
  const onSubmit = async (formData) => {
    ...
    aquaData.append('plantImage', formData.plantImage);
    aquaData.append('plantSmallImage', formData.plantSmallImage);
    ...
    try {
      ...
      toast.success('Aqua item added successfully!');
      reset(); // Reset the form fields
      router.push('/aqua-admin'); // Redirect to the appropriate page
    } catch (error) {
      toast.error(`Failed to add aqua item: ${error.message}`);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="bg-white shadow-lg rounded-lg max-w-md w-full p-6">
        <h1 className="text-2xl font-bold mb-4 text-center text-black">Add New Aqua Item</h1>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            >
              {isSubmitting ? 'Adding...' : 'Add Aqua Item'}
            </button>
          </div>
        </form>
      </div>
