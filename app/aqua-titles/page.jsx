...
export default function AquaTitlesHomeAdmin() {
    ...
    async function deleteAquaTitleItem(id) {
        try {
            const response = await fetch(`/api/aqua-titles/${id}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
                throw new Error('Failed to delete aqua title item');
            }
            setAquaTitlesItems(items => items.filter(item => item.id !== id));
            toast.success('Aqua title item deleted successfully');
        } catch (error) {
            console.error('Error deleting aqua title item:', error);
            toast.error('Failed to delete aqua title item');
        }
    }

    const handleAquaTitleItemClick = (itemId) => {
        router.push(`/aqua-titles/edit/${itemId}`);
    };
            ...
            <div className="min-h-screen flex flex-col bg-gray-100">
                ...
                <div className="flex flex-1">
                    ...
                    <div className="flex-grow p-6">
                        <h1 className="text-3xl font-bold mb-4 text-black">Aqua Titles</h1>
                        <Link href="/aqua-titles/add">
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full bg-white shadow-md rounded overflow-hidden">
                                ...
                                <tbody>
                                            ...
                                            <tr key={item.id} className="border-t text-black">
                                                ...
                                                <td className="py-2 px-4">
                                                    <button 
                                                        ...
                                                        onClick={() => handleAquaTitleItemClick(item.id)}
                                                    >
                                                        Edit
                                                    </button>
                                                    <button
                                                        onClick={() => deleteAquaTitleItem(item.id)}
                                    ...
                                    {!aquaTitlesItems.length && !isLoading && (
                                        <tr>
