...
const Page = () => {
  ...
  const [activeTitle, setActiveTitle] = useState("Inland Fishery Support");
      ...
      useEffect(() => {
        const fetchTitlesAndSubtext = async () => {
          try {
            ...
            await Promise.all([
              setTitleAndSubtext("Aqua First", setTitle1),
              setTitleAndSubtext("Aqua Second", setTitle2, setSubText),
            ]);
          } catch (error) {
        ...
        const fetchAquaItemsAndTitles = async () => {
          try {
            // Fetch agriculture titles
            const titleResponse = await fetch("/api/aqua-titles");
            if (!titleResponse.ok) throw new Error("Failed to fetch aqua titles");
            const titleData = await titleResponse.json();
            setAquaTitlesItems(titleData.data);
            ...
            const itemsResponse = await fetch("/api/aqua-admin");
            if (!itemsResponse.ok) throw new Error("Failed to fetch aqua items");
            const itemsData = await itemsResponse.json();
            setAquaItems(itemsData.data);
