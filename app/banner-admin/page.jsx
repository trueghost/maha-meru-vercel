...
export default function BannerAdmin() {
    ...
    useEffect(() => {
        const fetchBannerItems = async () => {
            try {
                ...
                setBannerItems(data.data); // Adjusted based on API response
            } catch (error) {
                console.error('Error fetching banner items:', error);
                toast.error('Failed to load banner items');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    async function deleteBannerItem(id) {
        try {
            ...
            if (!response.ok) {
                throw new Error('Failed to delete banner item');
            }
            setBannerItems(items => items.filter(item => item.id !== id));
            toast.success('Banner item deleted successfully');
        } catch (error) {
            console.error('Error deleting banner item:', error);
            toast.error('Failed to delete banner item');
        }
    }

    const handleBannerItemClick = (itemId) => {
        router.push(`/banner-admin/edit/${itemId}`);
    };
            ...
            <div className="min-h-screen flex flex-col bg-gray-100">
                ...
                <div className="flex flex-1">
                    ...
                    <div className="flex-grow p-6">
                        <h1 className="text-3xl font-bold mb-4 text-black">Banner Admin</h1>
                        <Link href="/banner-admin/add">
                                        ...
                                        bannerItems.map(item => (
                                            <tr key={item.id} className="border-t text-black">
                                                <td className="py-2 px-4">
                                                    <img src={`${item.image}`} alt={`Banner ${item.id}`} className="w-16 h-16 object-cover" />
                                                </td>
                                                <td className="py-2 px-4 truncate max-w-xs">{item.title}</td>
                                                <td className="py-2 px-4 truncate max-w-xs">{item.description}</td>
                                                <td className="py-2 px-4">
                                                    <button className="btn btn-sm btn-primary me-1 inline-block bg-blue-500 text-white py-1 px-2 rounded hover:bg-blue-600 mr-2" onClick={() => handleBannerItemClick(item.id)}>
                                                        Edit
                                                    </button>
                                                    <button
