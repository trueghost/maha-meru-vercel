...
export default function AddBlogItem() {
  ...
  const onSubmit = async (formData) => {
    ...
    blogData.append('link', formData.link); // Append the link to the FormData
    ...
    try {
      ...
      toast.error(`Failed to add blog item: ${error.message}`);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="bg-white shadow-lg rounded-lg max-w-md w-full p-6">
        <h1 className="text-2xl font-bold mb-4 text-center text-black">Add New Blog Item</h1>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            >
              {isSubmitting ? 'Adding...' : 'Add Blog Item'}
            </button>
          </div>
        </form>
      </div>
