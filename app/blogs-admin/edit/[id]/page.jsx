...
export default function EditBlogItem({ params }) {
        ...
        const fetchBlogItem = async () => {
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`Error ${response.status}: ${errorData.message || 'Failed to fetch blog item'}`);
                }
                ...
                if (data && data.data) {
                    ...
                    toast.error('No data found for the blog item.');
                }
            } catch (error) {
                console.error('Error fetching blog item:', error);
                toast.error(`Failed to load blog item: ${error.message}`);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const onSubmit = async (formData) => {
        ...
        blogData.append('link', formData.link); // Append the link to the form data
        
        if (formData.image) {
            blogData.append('image', formData.image[0]); // Only get the first file
        }
        ...
        try {
            ...
            toast.error(`Failed to update blog item: ${error.message}`);
        }
    };
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-100 p-4">
            <h1 className="text-2xl font-bold mb-4 text-center">Edit Blog Item</h1>
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Enter blog item category"
                    />
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Enter blog item link"
                    />
