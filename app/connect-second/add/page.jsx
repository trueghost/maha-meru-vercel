...
export default function AddConnectSecondItem() { // Updated component name
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const validationSchema = Yup.object().shape({
    title: Yup.string().required('Title is required'), // Added title validation
  ...
  const onSubmit = async (formData) => {
    ...
    connectData.append('title', formData.title); // Append title to FormData
    connectData.append('description', formData.description);
    connectData.append('image', formData.image);

    try {
      const response = await fetch('/api/connect-second', { // Updated API endpoint
        method: 'POST',
        body: connectData,
      });
      ...
      router.push('/connect-second'); // Redirect to the appropriate page
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <div className="bg-white shadow-lg rounded-lg max-w-md w-full p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Title</label> {/* New title input */}
            <input
              type="text"
              {...register('title')}
              ...
              placeholder="Enter connect item title" // Placeholder for title
            />
