...
export default function FormHandleAdmin() {
    ...
    useEffect(() => {
        const fetchFormHandles = async () => {
            try {
                ...
                // Reverse the order based on id
                const reversedData = [...data.data].reverse();
                setFormHandles(reversedData); // Set reversed data
                setFilteredFormHandles(reversedData); // Set filtered data as well
            } catch (error) {
    ...
    useEffect(() => {
        // Filter form handles based on the search query
        const filtered = formHandles.filter(item =>
            item.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.email.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.number.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.interests.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    async function deleteFormHandle(id) {
        try {
            const response = await fetch(`/api/form-handle/${id}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
    ...
    const handleFormHandleClick = (itemId) => {
        router.push(`/form-handle/edit/${itemId}`);
    };
        ...
        <Protect>
            <div className="min-h-screen flex flex-col bg-gray-100">
                ...
                <div className="flex flex-1">
                    {isSidebarOpen && <SidebarAdmin totalItems1={formHandles.length} />}
                    <div className="flex-grow p-6">
                        <h1 className="text-3xl font-bold mb-4 text-black">Form Handle Admin</h1>
                        ...
                        <div className="mb-4">
                            <input
                                ...
                                placeholder="Search by Name, Email, Number, Interests, or Description"
                                value={searchQuery}
                            ...
                            <table className="min-w-full bg-white shadow-md rounded overflow-hidden">
                                <thead className="bg-gray-200 text-gray-600">
                                    <tr>
                                        ...
                                        <th className="py-2 px-4 text-left text-black">Number</th>
                                        <th className="py-2 px-4 text-left text-black">Interests</th>
                                ...
                                <tbody>
                                    {isLoading ? (
                                        ...
                                        filteredFormHandles
                                          .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)) // Sorting latest first based on timestamp
                                          .map(item => (
                                            <tr key={item.id} className="border-t text-black">
                                                <td className="py-2 px-4">{item.name}</td>
                                                <td className="py-2 px-4">{item.email}</td>
                                                <td className="py-2 px-4">{item.number}</td>
                                                <td className="py-2 px-4 max-w-xs">{item.interests}</td>
                                                <td className="py-2 px-4 max-w-xs flex-shrink-0 break-words whitespace-normal">{item.description}</td>
