...
export default function MissionHomeAdmin() {
    ...
    const [missionMobile, setMissionMobile] = useState([]);
    ...
    useEffect(() => {
        const fetchMissionMobile = async () => {
            try {
                ...
                setMissionMobile(data.data); // Adjusted based on API response
            } catch (error) {
    ...
    async function deleteMissionMobileItem(id) {
        try {
            ...
            setMissionMobile(items => items.filter(item => item.id !== id));
    ...
    const handleMissionMobileClick = (itemId) => {
        router.push(`/mission-mobile-admin/edit/${itemId}`);
    };
            ...
            <div className="min-h-screen flex flex-col bg-gray-100">
                ...
                <div className="flex flex-1">
                    ...
                    <div className="flex-grow p-6">
                        ...
                        <Link href="/mission-mobile-admin/add">
                            ...
                            <table className="min-w-full bg-white shadow-md rounded overflow-hidden">
                                <thead className="bg-gray-200 text-gray-600">
                                    <tr>
                                        ...
                                        <th className="py-2 px-4 text-left">Small Logo</th>
                                ...
                                <tbody>
                                    {isLoading ? (
                                        ...
                                        missionMobile.map(item => (
                                            <tr key={item.id} className="border-t text-black">
                                                <td className="py-2 px-4"><img src={item.logo} alt="Logo" className="w-16 h-16 object-cover" /></td>
                                                <td className="py-2 px-4"><img src={item.smallLogo} alt="Small Logo" className="w-16 h-16 object-cover" /></td>
                                                ...
                                                <td className="py-2 px-4">
                                                    <button className="btn btn-sm btn-primary me-1 inline-block bg-blue-500 text-white py-1 px-2 rounded hover:bg-blue-600 mr-2" onClick={() => handleMissionMobileClick(item.id)}>
                                                        Edit
                                                    </button>
                                                    <button
                                                        onClick={() => deleteMissionMobileItem(item.id)}
                                    ...
                                    {!missionMobile.length && !isLoading && (
                                        <tr>
                                            <td colSpan="19" className="py-4 text-center text-black">
