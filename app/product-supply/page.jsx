...
export default function ProductSupplyHomeAdmin() {
    const router = useRouter();
    const [productSupplyItems, setProductSupplyItems] = useState([]);
    ...
    useEffect(() => {
        const fetchProductSupplyItems = async () => {
            try {
                ...
                if (!response.ok) {
                    throw new Error('Failed to fetch product supply items');
                }
                const data = await response.json();
                // console.log(data.data);
                setProductSupplyItems(data.data); // Adjusted based on API response
            } catch (error) {
                console.error('Error fetching product supply items:', error);
                toast.error('Failed to load product supply items');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    async function deleteProductSupplyItem(id) {
        try {
            const response = await fetch(`/api/product-supply/${id}`, {
                method: 'DELETE',
            });
            if (!response.ok) {
                throw new Error('Failed to delete product supply item');
            }
            setProductSupplyItems(items => items.filter(item => item.id !== id));
            toast.success('Product supply item deleted successfully');
        } catch (error) {
            console.error('Error deleting product supply item:', error);
            toast.error('Failed to delete product supply item');
        }
    }

    const handleProductSupplyItemClick = (itemId) => {
        router.push(`/product-supply/edit/${itemId}`);
    };
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full bg-white shadow-md rounded overflow-hidden">
                                ...
                                <tbody>
                                            ...
                                            <tr key={item.id} className="border-t text-black">
                                                ...
                                                <td className="py-2 px-4">
                                                    <button 
                                                        ...
                                                        onClick={() => handleProductSupplyItemClick(item.id)}
                                                    >
                                                        Edit
                                                    </button>
                                                    <button
                                                        onClick={() => deleteProductSupplyItem(item.id)}
                                    ...
                                    {!productSupplyItems.length && !isLoading && (
                                        <tr>
