...
// Dynamically import components for lazy loading without SSR
const AnimationTrial = dynamic(() => import("./components/animationTrial"));
...
useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      ...
      setMissionItems(resolvedFirstMissionItem); // Set the object directly, not an array
      setProducts(fetchedTrendingProducts);
      setBlogPosts(fetchedBlogPosts);
      setAchievements(fetchedAchievements);

      titles.forEach((titleData) => {
        Object.entries(titleData).forEach(([key, title]) => {
  ...
  return (
    <main className="overflow-x-hidden h-screen w-screen relative">
      {isLoading && (
      ...
      <CookieBanner />
        ...
        <Image
          ...
          alt="sticky-logo"
          ref={logoRef}
          className="fixed top-10 left-[5%] z-50 bg-transparent 2xl:h-[116px] 2xl:w-[116px] lg:w-[80px] lg:h-[80px]"
          style={{
            visibility:
        ...
        <AnimationTrial missionItems={missionItems} translatedSeeProductsText={translatedSeeProductsText} />
        {/* <LogoAnimation /> */}
        <Agriculture title3={title3} subtext3={subtext3} items={items} hovered={hovered} setHovered={setHovered} />
        <TrendingProducts title4={title4} products={products} translatedLearnMoreText={translatedLearnMoreText} hovered={hovered} />
